...
def setup(frame, show_waypoints=True):
    env = frame.env
...
#    Use this for a squared map
    ...
    # Plots the "land"
    ax1.fill(np.array([-r*1.1, r*1.1, r*1.1, -r*1.1]),
             np.array([-r*1.1, -r*1.1, r*1.1, r*1.1]), "g")
    # Plots the water
    ax1.fill(x, y, 'b')
    ...
    # Creates waypoints (to be updated in "updplot")
    if show_waypoints:
        ax1.plot([], [], 'r+')
    ...
    ax4.set_yticklabels([])
    ax4.arrow(-180/360*np.pi*2, env.V, 180/360*np.pi *
              2, 0, head_width=-0.075, shape='right')
    ax4.arrow(-180/360*np.pi*2, env.V, 180/360*np.pi *
              2, 0, head_width=0.075, shape='left')
...
def updplot(frame):
    ...
    # Plots the navigator's waypoints
    if len(ax1.get_children()) > 6:
        waypoints = locator.get_navigator().view_waypoints()
        x = [waypoint[0] for waypoint in waypoints]
        y = [waypoint[1] for waypoint in waypoints]
        ax1.get_children()[6].set_xdata(x)
        ax1.get_children()[6].set_ydata(y)
